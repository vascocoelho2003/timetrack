...
router.post("/import/:team_id/:project_id", async (req, res) => {
  try {
    ...
    for (const row of rows) {
      ...
      if (tasklist == null) {
        db.prepare(`INSERT INTO task_lists (project_id, name) VALUES(?,?)`).run(
          project,
          row.tasklist,
        );
        tasklist = db
      ...
      task = db
        .prepare(
          "SELECT * FROM tasks WHERE title=? AND task_list_id=? AND due_date=?",
        )
        .get(row.tarefa, tasklist.id, dueDate);
      if (task) {
        continue;
      }
      ...
      insertTaskStmt = db.prepare(
        `INSERT INTO tasks(task_list_id,title,description,priority,due_date,created_by_user_id,next_alert_date, alert_offset_days) VALUES(?,?,?,?,?,?,?, ?)`,
      );
      const priority = row.priority ?? "medium";
      if (row.alert_date) {
        alert_date = new Date((row.alert_date - 25569) * 86400 * 1000)
          .toISOString()
          .split("T")[0];
      } else {
        alert_date = null;
      ...
      if (row.responsavel) {
        ...
        if (user && team_member) {
          ...
          ).run(task.lastInsertRowid, user.id);
        }
      }
