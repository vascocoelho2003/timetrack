...
function sanitizeProjectDescription(value = "") {
  return String(value ?? "")
    .trim()
    .slice(0, MAX_PROJECT_DESCRIPTION_LENGTH);
}
...
router.get("/team/:teamId", (req, res) => {
  ...
  const projects = db
    .prepare("SELECT * FROM projects WHERE team_id = ? ORDER BY name")
    .all(teamId);
...
router.post("/team/:teamId", (req, res) => {
  ...
  if (!isTeamAdmin(req.user.id, teamId)) {
    return res
      .status(403)
      .json({ error: "Apenas admins podem criar projetos" });
  }
  if (!name?.trim()) {
...
router.put("/:projectId", (req, res) => {
  ...
  if (!isTeamAdmin(req.user.id, project.team_id)) {
    return res
      .status(403)
      .json({ error: "Apenas admins podem editar projetos" });
  }
...
router.delete("/:projectId", (req, res) => {
  ...
  if (!isTeamAdmin(req.user.id, project.team_id)) {
    return res
      .status(403)
      .json({ error: "Apenas admins podem eliminar projetos" });
  }
...
router.get("/", (req, res) => {
  ...
  const projects = db
    .prepare(
      `SELECT p.* FROM projects p JOIN team_members tm ON p.team_id = tm.team_id WHERE tm.user_id = ?`,
    )
    .all(user_id);
