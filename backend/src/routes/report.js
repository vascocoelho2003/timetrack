...
router.get("/generalClientReport", authMiddleware, async (req, res) => {
  ...
  const startDate = toStartOfDay(
    req.query.startDate || req.body.startDate || `${currentYear}-01-01`,
  );
  const endDate = toEndOfDay(
    req.query.endDate ||
      req.body.endDate ||
  ...
  if (department) {
    ...
    client = db
      .prepare(`SELECT id FROM clients WHERE department_id = ?`)
      .get(dep.department_id);
    if (!client) {
      return res
        .status(404)
        .json({ error: "Cliente de departamento não encontrado." });
    }
  } else {
    client = db
...
router.get(
  ...
  async (req, res) => {
    ...
    const currentYear = new Date().getFullYear();
    const startDate = toStartOfDay(
      req.query.startDate || `${currentYear}-01-01`,
    );
    const endDate = toEndOfDay(
      req.query.endDate || new Date().toISOString().slice(0, 10),
    );
