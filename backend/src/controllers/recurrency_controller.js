...
/**
 * Obtém a regra de recorrência de uma tarefa
 * @param {*} taskOrTaskId
 * @returns
 */
function getRecurrency(taskOrTaskId) {
  const taskId =
    typeof taskOrTaskId === "object" ? taskOrTaskId.id : taskOrTaskId;
  return db
...
/**
 * Obtém os responsáveis por uma tarefa
 * @param {*} taskId
 * @returns
 */
...
/**
 * Verifica se uma tarefa tem uma regra de recorrência
 * @param {*} taskId
 * @returns
 */
...
/**
 * Clona uma tarefa para uma nova data
 * @param {*} task
 * @param {*} date
 * @param {*} sourceTaskId
 * @param {*} users
...
function cloneTask(task, date, sourceTaskId, users, logged_user_id) {
  ...
  if (
    sourceTask.alert_offset_days > 0 &&
    sourceTask.alert_offset_days != null
  ) {
  ...
  const result = db
    .prepare(
      `INSERT INTO tasks (task_list_id, parent_task_id, title, description, priority, due_date, created_by_user_id, next_alert_date, alert_offset_days) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    )
    .run(
      task.task_list_id,
      task.id,
  ...
  db.prepare(`UPDATE recurrence_rules SET task_id = ? WHERE task_id = ?`).run(
    result.lastInsertRowid,
    sourceTaskId,
  );
  users.forEach((user) => {
    db.prepare(`INSERT INTO task_assignees (task_id, user_id) VALUES(?,?)`).run(
      result.lastInsertRowid,
      user.user_id,
    );
  });
...
/**
 * Obtém o tipo de recorrência de uma tarefa
 * @param {*} taskId
 * @returns
 */
...
/**
 * Cria uma regra de recorrência para uma tarefa
 * @param {*} taskId
 * @param {*} userid
 */
  ...
  switch (recurrenceRule.rule_type) {
    ...
    case "business_day":
      if (recurrenceRule.frequency != "monthly") {
        const businessDate = calculateBusinessDay(
          recurrenceRule,
          task.due_date,
        );
...
/**
 * Obtém o n-ésimo dia de trabalho de um mês
 * @param {*} year
 * @param {*} month
 * @param {*} n
 * @returns
...
/**
 * Obtém o número de dias de trabalho de um mês
 * @param {*} date
 * @returns
 */
...
/**
 * Calcula a data de uma tarefa fixa
 * @param {*} rule
 * @param {*} currentDate
 * @returns
 */
...
/**
 * Calcula a data de uma tarefa recorrente
 * @param {*} rule
 * @param {*} currentDate
 * @returns
 */
