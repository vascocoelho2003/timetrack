...
/**
 * Verifica se o servidor de email está funcionando
 */
...
/**
 * Busca as tarefas que vencem dentro de uma semana e envia notificações por email
 */
...
/**
 * Envia notificações por email para os responsáveis das tarefas
 * @param {*} tasks
 */
async function sendEmailNotifications(tasks) {
  ...
  for (const task of tasks) {
    try {
      ...
      console.log(`Tarefa "${task.title}" atribuída a:`, users);

      if (users.length === 0) {
        console.log(`A tarefa ${task.id} não tem responsáveis`);
        continue;
      }
      ...
      console.log(`Tarefa ${task.id} marcada como notificada`);
    } catch (error) {
      console.error(`Erro ao enviar notificação da tarefa ${task.id}:`, error);
    }
  }
}
